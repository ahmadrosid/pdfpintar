...
<div class="p-4 group {message.role === 'user' ? 'bg-neutral-100 dark:bg-neutral-700/70' : 'bg-white dark:bg-neutral-800'}">
    ...
    <div class="pt-2 justify-end flex gap-3 items-center">
        <button class="opacity-70 hover:opacity-100 p-1 invisible group-hover:visible text-xs flex gap-2 items-center">
            {#if copied}
                <CopyCheckIcon class="size-3" /> Copied
            {:else}
                <CopyIcon onclick={() => copyMessage(message.content)} class="size-3" /> Copy
            {/if}
        </button>
        <button class="flex gap-2 items-center text-xs opacity-70 hover:opacity-100 invisible group-hover:visible">
